...
Auto-Apply appears at the bottom of the companion only while an app is connected and is off by default on first launch. The companion remembers the setting across restarts, including restarts from the `+ Tinkerble` scheme. When enabled, Boolean and enum changes write immediately. Numeric fields wait two seconds after the last typed value or arrow-key adjustment, while slider-style drags write only when the drag ends. Text, color, and date controls also use the delay to avoid repeated source writes while their values are still changing.

After Tinkerble successfully applies a value, Xcode may not immediately refresh an editor that already has the modified file open. This is a limitation of how Xcode notices changes made by external processes and does not mean the source edit was lost. The file is updated on disk, and the change will appear in Xcode's Source Control diff or in `git diff`, even if the editor still shows the previous value. Leaving the file and reopening it will refresh the editor.
